...
\begin{itemize}
\item A point $p$ is a core point if at least $minPts$ points are within distance ? of it (in it's neighborhood). Those points are said to be directly reachable from $p$. No points are reachable from a non-core point.
...
In this diagram, $minPts = 3$. Point A and the other red points are core points, because at least three points surround it in an ? radius. Because they are all reachable from one another, they form a single cluster. Points B and C are not core points, but are reachable from A (via other core points) and thus belong to the cluster as well. Point N is a noise point that is neither a core point nor density-reachable.
