...
\par For completeness, we have included the Python code for \texttt{DBSCAN} we used in this project below. 
...
\doublespacing
\par The density checks mentioned above are implemented as a call to \texttt{regionQuery}, which check the points in the \texttt{eps}-neighborhood. Clusters grow, after getting identified, by a call to \texttt{expandCluster}, which marks a point as noise if it has too few neighbors, or otherwise adds it to a list of core points that will be recursively expanded if the density requirements are met at each iteration.
...
In addition, the \texttt{expandCluster} routine is only called from a single location, so it may be safely inlined. In the interest of simply surveying these clustering algorithms, we have not pursued these optimizations.
